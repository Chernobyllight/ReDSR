...
from TRAIN.TRAIN_setting2.DA_MODEL import DANet
...
if __name__=='__main__':
    ...
    with torch.no_grad():
        ...
        for idx_img, (val_hr) in enumerate(val_loader):
            ...
            val_lr,kernel = degrade(val_hr, random=False)  # 1, c, h//4, w//4
        ...
        ave_val_psnr_wlg = eval_psnr_wlg / len(val_loader)
        ave_val_ssim_wlg = eval_ssim_wlg / len(val_loader)
        print('val psnr wlg:', ave_val_psnr_wlg)
        print('val ssim wlg:', ave_val_ssim_wlg)


        kernel = torch.squeeze(kernel,dim=0)
